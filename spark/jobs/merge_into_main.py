...
# sql=f"""
# INSERT INTO {NESSIE_CATALOG_NAME}{NAMESPACE}.orders
# SELECT 
#     * 
# FROM 
...
try:
    ...
    # spark.sql(f"SELECT COUNT(*) AS main_orders_cnt_b4_insert FROM {NESSIE_CATALOG_NAME}{NAMESPACE}.orders").show()
    # spark.sql(sql)
    ...
    # spark.sql(f"SELECT COUNT(*) AS main_orders_cnt_after_insert FROM {NESSIE_CATALOG_NAME}{NAMESPACE}.orders").show()
    # spark.sql(f"SELECT * FROM {NESSIE_CATALOG_NAME}{NAMESPACE}.orders").show()
