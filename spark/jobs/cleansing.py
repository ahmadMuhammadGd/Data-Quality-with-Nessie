...
try:    
    ...
    # prepared sql
    sql = f"""
    CREATE OR REPLACE TABLE {NESSIE_CATALOG_NAME}.{NAMESPACE}.{SILVER_AMAZON_SALES_TABLE}_temp
    USING ICEBERG AS
    SELECT
        {', '.join([
...
\n''', sql)
    ...
    print('Source df:', df.count())
    print('Stage1: cleaned_df:', clean_df.count())
    print('Stage2: cleaned_df:', spark.sql(f"SELECT count(*) AS cnt FROM {NESSIE_CATALOG_NAME}.{NAMESPACE}.{SILVER_AMAZON_SALES_TABLE}_temp").collect()[0]['cnt'])
