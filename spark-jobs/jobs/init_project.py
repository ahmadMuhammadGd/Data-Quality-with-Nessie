...
try:
    spark.sql(f"CREATE NAMESPACE IF NOT EXISTS {NESSIE_CATALOG_NAME}.{NAME_SPACE}")
    spark.sql(f"CREATE NAMESPACE IF NOT EXISTS {NESSIE_CATALOG_NAME}.keys")
    
    print("Namespaces .. done")
    spark.sql(f"DROP TABLE IF EXISTS {NESSIE_CATALOG_NAME}.{NAME_SPACE}.{BRONZE_AMAZON_SALES_TABLE}")
    spark.sql(f"DROP TABLE IF EXISTS {NESSIE_CATALOG_NAME}.{NAME_SPACE}.{SILVER_AMAZON_SALES_TABLE}")
    
    spark.sql(
        f"""
        CREATE TABLE IF NOT EXISTS {NESSIE_CATALOG_NAME}.{NAME_SPACE}.{BRONZE_AMAZON_SALES_TABLE} (
            Order_ID            string,
            order_Date          Date,
            Status              string,
            Fulfilment          string,
    ...
    spark.sql(
        f"""
        CREATE TABLE IF NOT EXISTS {NESSIE_CATALOG_NAME}.{NAME_SPACE}.{SILVER_AMAZON_SALES_TABLE} (
            Order_ID            string,
            order_Date          Date,
            Status              string,
            Fulfilment          string,
